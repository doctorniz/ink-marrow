...
| **Board View** | Quick-capture notice board (`Ctrl+2`). Items called **Thoughts** are `.md` files in `_marrow/_board/` (hidden from file tree/browser/search). Frontmatter stores `type` (future: bookmark, list, reminder, task, audio), `color`, timestamps. Title derived from the first `# H1` line if present. Masonry CSS-columns layout with pastel-tinted cards. Click to edit inline via a minimal Tiptap instance (bold/italic/underline/lists via keyboard shortcuts only — no toolbar, no font changes). Image thoughts: image stored in `_marrow/_board/_assets/`, rendered as card with embedded preview. `useBoardStore` (`stores/board.ts`) manages CRUD; `lib/board/index.ts` handles parse/serialize; `lib/editor/board-extensions.ts` provides the slim extension set. |
